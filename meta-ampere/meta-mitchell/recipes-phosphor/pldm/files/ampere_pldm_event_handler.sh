...
#Indicates an Ampere critical event. Used for the critical errors
...
# Uncorrectable RAS Error sensor IDs
    ...
    case $byte3 in
        ...
        $((0x15)) )
            if [[ $byte1 -eq 0 ]] && [[ $byte2 -eq 0 ]]; then
                local isFailed=$(( byte0 & 0x1 ))
                if [[ isFailed -ne 0 ]]; then
                    echo "$ERROR_MSG:SPD/HUB high temp condition"
                else
...
case $eventType in
    ...
    $((PLDM_MESSAGE_POLL_EVENT)) )
        ...
        ;;
    *)
        echo "Error: Unsupported EventType $eventType"
        exit 0
        ;;
esac
