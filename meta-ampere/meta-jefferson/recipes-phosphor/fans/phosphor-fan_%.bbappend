...
JEFFERSON_COMPAT_NAME = "com.ampere.Hardware.Chassis.Model.MtJefferson"
...
do_install:append () {
        ...
        # datadir = /usr/share
        install -d ${D}${datadir}/phosphor-fan-presence/control/${JEFFERSON_COMPAT_NAME}
        install -d ${D}${datadir}/phosphor-fan-presence/monitor/${JEFFERSON_COMPAT_NAME}
        install -d ${D}${datadir}/phosphor-fan-presence/presence/${JEFFERSON_COMPAT_NAME}

        for CONTROL_CONFIG in ${CONTROL_CONFIGS}
        do
                install -m 0644 ${WORKDIR}/${CONTROL_CONFIG} \
                        ${D}${datadir}/phosphor-fan-presence/control/${JEFFERSON_COMPAT_NAME}
        done

        install -m 0644 ${WORKDIR}/monitor.json \
                ${D}${datadir}/phosphor-fan-presence/monitor/${JEFFERSON_COMPAT_NAME}/config.json
        install -m 0644 ${WORKDIR}/presence.json \
                ${D}${datadir}/phosphor-fan-presence/presence/${JEFFERSON_COMPAT_NAME}/config.json
}
